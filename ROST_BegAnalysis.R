...
#banding date data
band<-read.table("ROST_BegBandingData_Github.csv",header = TRUE,sep = ",",as.is=TRUE)
##############################Parent-offspring association analysis############################################
...
Day.fig<-ggplot(newdata, aes(x = Day, y = fit)) + geom_ribbon(aes(ymin = lwr,
  ...
  geom_line(size = 1.2, color = "dodgerblue4") + ylim(c(0, 3.2))+ #change upper ylim to 15 to expand y-range to full data extent
...
Time.fig<-ggplot(newdata.t, aes(x = time, y = fit)) + 
  stat_sum(data = Focals, aes(x= scale(TIME), y = beg)) +
  geom_ribbon(aes(ymin = lwr,ymax = upr),alpha=0.2, fill="dodgerblue4") + 
  geom_line(size = 1.2, color="dodgerblue4") + ylim(c(0, 2.2))+xlim(c(7,19))+ #same as day fig, adjust ylim to 15 for full data extent
...
#Figure manipulation for publication was completed in Inkscape software.
